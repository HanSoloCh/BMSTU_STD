...
Если проверки не пройдут, коммит будет заблокирован (кроме случая использования флага `--no-verify`).
...
Цикломатическая сложность метода не должна превышать 10. Если метод превышает этот порог, его нужно разбить на более мелкие методы.
...
**Внимание**: Используйте `--no-verify` только в исключительных случаях и обязательно исправьте проблемы в следующем коммите.
...
# Автоисправление
./gradlew detektFormat ktlintFormat
